...
class MusicSpider(Spider):
    ...
    def start_requests(self):
        for cat in self.cats: 
            ...
            url = self.base_url+'/discover/playlist/?{cat}'.format(cat=urlencode(catDict))
    ...
    def getSheetPageNum(self, response):
        ...
        for offset in offsets:
                catDict = {'cat': cat.encode('utf8')}
                url = '{url}/discover/playlist/?order=hot&{cat}&limit=35&offset={offset}'.format(url=self.base_url, cat=urlencode(catDict), offset=offset)
